...
void testTone(int c, int fl, int fr, float d)
{
	...
	// all the argumetns are okay, the rest of code is to
...
// The function finds how many peaks from 80-pieces of decibel values
int findPeaks(int d[]){
	int c = 0, i;
	for (i=1; i<80; i++){
...
//this function gets one second of samples (16000), and calculates 80 pieces of decibel value,
//we know we need to calculate one decibel value from 200 samples, decibel value is calculated
//by RMS (Rout mean square) formula.
